...
@section('content')
    <form action="/alternatif/{{ $item->kode_alternatif }}" method="POST" class="mt-1" enctype="multipart/form-data">
        @method('put')
        @csrf
        <div class="form-control">
            ...
            <input type="text" name="nama_alternatif" value="{{ old('nama_alternatif', $item->nama_alternatif) }}"
        ...
        <div class="form-control">
            ...
            <select class="select select-sm select-bordered" name="kode_kategori">
                @foreach ($kategori as $kateg)
                    <option @if ($kateg->kode_kategori == $item->kode_kategori) selected @endif value="{{ $kateg->kode_kategori }}">
